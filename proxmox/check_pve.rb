...
class CheckPve
  ...
  ###--- SMART CHECK ---###
  def smart # rubocop:disable Metrics/AbcSize
  ...
  def subscription
    ...
    check_single_data(data: data['status'] != 'Active', message: data['message'])
    check_multiple_data(
  ...
  def node_helper_to_pct(value:, output_msg:, perf_label: 'Usage', path: 'status', **args)
  ...
  def node_helper_to_units(type:, output_msg:, value: 'used', path: 'status', unit: 'gb', perf_label: 'Usage')
    http_connect(path: "api2/json/nodes/#{@options[:node]}/#{path}")
  ...
  def rrddata_helper(value:, output_msg:, unit: 'kb', perf_label: 'Usage')
    check_rrddata_path
